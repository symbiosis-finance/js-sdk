...
export class ZappingBtc2 extends BaseSwapping {
    ...
    protected async buildTradeC(amountIn: TokenAmount, amountInMin: TokenAmount): Promise<SymbiosisTrade> {
        let amountOut = this.transit.amountOut
        let amountOutMin = this.transit.amountOutMin

        if (!amountIn.token.equals(this.syBtc)) {
            const aggTrade = await new AggregatorTrade({
                ...
                to: this.multicallRouter.address as Address, // ?
        ...
        const [minBtcFee, threshold] = await Promise.all([
            getToBtcFee(amountOut, this.synthesis, this.symbiosis.cache),
            getThreshold(amountOut, this.synthesis, this.symbiosis.cache),
        ])
        ...
        const volumeFeeCollector = this.symbiosis.getVolumeFeeCollector(amountOut.token.chainId, [ChainId.BTC_MAINNET])
    ...
    public async exactIn({
        ...
        const result = await this.doExactIn({
            ...
            transitTokenOut,
            partnerAddress,
            depositoryAllowed: false, // do not allow building depository trade instead of a straight trade
        })
