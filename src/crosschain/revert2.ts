...
export class RevertRequest {
    constructor(private symbiosis: Symbiosis, private transactionHash: string, private chainId: ChainId) {}
...
export class Revert2 {
    constructor(private symbiosis: Symbiosis, private request: RevertRequest) {}
    ...
    async waitForComplete() {
        const { chainId } = this.request

        const externalId = this.getExternalId()

        let filter: Filter
        if (type === 'synthesize') {
            const otherPortal = this.symbiosis.portal(chainId)
            filter = otherPortal.filters.RevertSynthesizeCompleted(externalId)
        } else {
            const otherSynthesis = this.symbiosis.synthesis(chainId)
            filter = otherSynthesis.filters.RevertBurnCompleted(externalId)
        }

        const log = await getLogWithTimeout({ chainId: chainId, filter, symbiosis: this.symbiosis })
