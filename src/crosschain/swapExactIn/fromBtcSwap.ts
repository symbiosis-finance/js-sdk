...
export async function fromBtcSwap(
    ...
    const allConfigs = symbiosis.config.btcConfigs.filter((i) =>
        i.symBtc.chainId !== ChainId.ZKSYNC_MAINNET
    );
    // prefer to use destination chain syBTC to avoid cross-chain routing
    const chainOutConfigs = allConfigs.filter((i) =>
        i.symBtc.chainId === tokenOut.chainId
    );
...
async function fromBtcSwapInternal(
    context: SwapExactInParams,
    btcConfig: BtcConfig,
): Promise<SwapExactInResult> {
    const {
    ...
    const syBtc = symbiosis.tokens().find((token) =>
        token.equals(syBtcSynth) && !token.isSynthetic
    );
    if (!syBtc) {
    ...
    // >> PORTAL FEE
    const btcPortalFeeRaw = await getBtcPortalFee(
        forwarderUrl,
        symbiosis.cache,
    );
    ...
    const { tail: initialTail } = await buildTail(
        context,
        btcConfig,
        syBtcAmount,
    );
    ...
    symbiosis.context?.logger.info(
        "Should be minted not less than",
        `${syBtcAmount.toSignificant()} syBTC`,
    );
...
async function buildTail(
    ...
    const feeCollector = symbiosis.getVolumeFeeCollector(
        syBtcAmount.token.chainId,
        [ChainId.BTC_MAINNET],
    );
    if (feeCollector) {
    ...
    const multicallCalldata = multicallRouter.interface.encodeFunctionData(
        "multicall",
        [
            "0", // will be patched
            [...calls.map((i) => i.data)],
    ...
    const symBtcContract = SymBtc__factory.connect(
        symBtc.address,
...
function erc20TransferCall(to: Address, tokenOut: Address): CallData {
    ...
    return {
        target: tokenOut,
        targetCalldata: ERC20__factory.createInterface().encodeFunctionData(
            "transfer",
            [to, 0n],
        ),
...
function nativeUnwrapCall(
    ...
    return {
        ...
        targetCalldata: IRouter__factory.createInterface().encodeFunctionData(
            "transferNative",
            [
                tokenOut.address,
                to,
...
async function buildOnChainSwap(
    ...
    if (dep && dep.cfg.priceEstimation.enabled) {
        ...
        tokenAmountOut = syBtcAmount.convertTo(
            context.tokenOut,
            (syBtcPrice / tokenOutPrice) *
                (1 - dep.cfg.priceEstimation.slippageNorm),
        );
        tokenAmountOutMin = syBtcAmount.convertTo(
            context.tokenOut,
            (syBtcPrice / tokenOutPrice) *
                (1 - dep.cfg.priceEstimation.slippageMax),
        );
    } else {
        aggregatorTrade = new AggregatorTrade({
            ...
            clientId: symbiosis.clientId,
            preferOneInchUsage: isUseOneInchOnly(
                tokenAmountIn.token,
                context.tokenOut,
            ),
        });
...
async function buildCrossChainSwap(
    ...
    const data = (swapExactInResult.transactionRequest as TransactionRequest)
        .data!;
    const result = MetaRouter__factory.createInterface().decodeFunctionData(
        "metaRoute",
        data,
    );
    const tx = result
        ._metarouteTransaction as MetaRouteStructs.MetaRouteTransactionStruct;
...
async function buildDepositCall({
    ...
    if (
        ...
        dep.btcRefundUnlocker !== undefined
    ) {
    ...
    const lockTx = await dep.depository.populateTransaction.lock(
        deposit,
        unlocker,
    );
...
async function getBtcPortalFee(
    forwarderUrl: string,
    cache: Cache,
): Promise<string> {
    ...
    try {
        const fastestFee = await cache.get(
            ["getFastestFee"],
            getFastestFee,
            60,
        ); // 1 minute
